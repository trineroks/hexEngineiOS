...
CEnvironment::CEnvironment(int *board, int playerNum, int p_boardW, int p_boardH, CSDL_Setup* p_SDL_setup)
{
	...
	for (int i=0; i<mapSize; i++)
	{
		...
		{
		case 1:
			hexes_[i].initializeValues("DIRT", MINIMUM_MOVE_COST, MINIMUM_MOVE_COST, MINIMUM_MOVE_COST, 0, false);
			break;
		case 2:
			hexes_[i].initializeValues("MOUNTAIN", MINIMUM_MOVE_COST, 5, NON_PASSABLE, 4, true);
			break;
		case 3:
			hexes_[i].initializeValues("WATER", MINIMUM_MOVE_COST, NON_PASSABLE, NON_PASSABLE, 0, false);
			break;
		}
	}
	...
	//CWeapon primary(8, 10, 1, false, 0, true, false, false, "M16A2", true, 30, 0, true, false);
	//CWeapon support(3, 30, 2, true, 2, false, false, true, "LAAD Stinger-C", false, 50, 0, false, true);
...
CEnvironment::~CEnvironment(void)
{
	...
	delete actionMenu_;
	delete cursor;
	for (vector<CUnit*>::iterator i = p1units.begin(); i != p1units.end(); i++)
	{
		delete (*i);
		(*i) = NULL;
	}
	for (vector<CUnit*>::iterator i = p2units.begin(); i != p2units.end(); i++)
	{
		delete (*i);
		(*i) = NULL;
	}
...
void CEnvironment::getAdjacentHexes(int x, int y, gamePoint adjacent[])
{
	for (int i=0; i<6; i++)
	{
		adjacent[i].x = x;
...
void CEnvironment::draw(float camX, float camY)
{
	...
	for (int i = 0; i < height; i++)
	{
		for (int j = 0; j < width; j++)
		{
			...
			drawWithParameters(imageCamera_, localBoard[j + width*i]);
		}
	}
}

void CEnvironment::drawWithParameters(SDL_Rect imageCamera, int tileType)
{
	crop_.w = HEX_SIZE;
	crop_.h = HEX_SIZE;
	if (tileType == 1)
...
void CEnvironment::selectedStateToggle(gamePoint gameMouse)
{
	if (actionMenu_ == NULL)
		return;
	else
...
void CEnvironment::actUponSelect(int x, int y, float camX, float camY)
{
	...
	else if (hexes_[index].existsUnit() && hexes_[index].getUnit()->getOwner() == currentPlayer)
	{
		...
		else if (currentlySelectedUnit_ != NULL)
		{
...
gamePoint CEnvironment::hitTest(gamePoint gameMouse)
{
	gamePoint result;
...
bool CEnvironment::hitTestMenu(gamePoint gameMouse)
{
	if (actionMenu_ == NULL)
		return false;
...
void CEnvironment::HUDButtonToggle(gamePoint gameMouse)
{
	if (endTurnButton_->selected(gameMouse))
		endTurnButton_->stateSelected();
}

void CEnvironment::HUDButtonAction(gamePoint gameMouse)
{
	if (endTurnButton_->selected(gameMouse))
		doAction(endTurnButton_->getActionToDo());
}

bool CEnvironment::hitTestButton(gamePoint gameMouse)
{
    if (endTurnButton_->selected(gameMouse))
        return true;
    return false;
...
bool CEnvironment::blockedLineOfSight(int originX, int originY, int targetX, int targetY)
{
	...
	checkedTiles.push_back(hexes_[originX + width*originY]);
	...
	while (current.x != targetX || current.y != targetY)
	{
		...
		if (hexes_[current.x + width*current.y].getIsObstacle())
			return true;
		checkedTiles.push_back(hexes_[current.x + width*current.y]);
	}
	return false;
}

Point CEnvironment::getAdjacentHexAtDirection(int x, int y, int i_dir, int j_dir)
{
	Point adjacent;
	adjacent.x = x;
	adjacent.y = y;
